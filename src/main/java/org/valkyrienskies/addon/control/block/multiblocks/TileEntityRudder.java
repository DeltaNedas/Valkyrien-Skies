...
public class TileEntityRudder extends
    TileEntityMultiblockForce<RudderMultiblockSchematic, TileEntityRudder> {
    ...
    private Vector getForcePosRelativeToAxleInShipSpace() {
        if (getRudderSchematic().isPresent()) {
            Vec3i directionFacing = getRudderFacingDirection().get().getDirectionVec();
            Vec3i directionAxle = this.getRudderAxisDirection().get().getDirectionVec();
            Vector facingOffset = new Vector(directionFacing.getX(), directionFacing.getY(),
                directionFacing.getZ());
            double axleLength = getRudderLength().get();
    ...
    public Optional<EnumFacing> getRudderAxisDirection() {
        Optional<RudderMultiblockSchematic> rudderAxleSchematicOptional = getRudderSchematic();
    ...
    public Optional<EnumFacing> getRudderFacingDirection() {
        Optional<RudderMultiblockSchematic> rudderAxleSchematicOptional = getRudderSchematic();
        if (rudderAxleSchematicOptional.isPresent()) {
    ...
    public Optional<Integer> getRudderLength() {
        Optional<RudderMultiblockSchematic> rudderAxleSchematicOptional = getRudderSchematic();
        if (rudderAxleSchematicOptional.isPresent()) {
    ...
    private Optional<RudderMultiblockSchematic> getRudderSchematic() {
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (this.isPartOfAssembledMultiblock() && this.isMaster() && getRudderAxisDirection()
            .isPresent()) {
            BlockPos minPos = this.pos;
            EnumFacing axleAxis = getRudderAxisDirection().get();
            EnumFacing axleFacing = getRudderFacingDirection().get();
            ...
            int axleLength = getRudderLength().get();
    ...
    public boolean attemptToAssembleMultiblock(World worldIn, BlockPos pos, EnumFacing facing) {
		List<IMultiblockSchematic> schematics = MultiblockRegistry.getSchematicsWithPrefix(EnumMultiblockType.RUDDER.toString());
        for (IMultiblockSchematic schematic : schematics) {
            RudderMultiblockSchematic rudderSchem = (RudderMultiblockSchematic) schematic;
            if (facing.getAxis() != rudderSchem.getAxleAxisDirection().getAxis()
                && rudderSchem.getAxleFacingDirection() == facing
                && schematic.attemptToCreateMultiblock(worldIn, pos, EnumMultiblockType.RUDDER)) {
                return true;
            }
        }
        return false;
